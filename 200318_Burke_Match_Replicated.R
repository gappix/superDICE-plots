...
# shortcuts
oldBAUdamSSP3 = PPstory5$v1x00$SIMUL$ed57$ssp3$noncoop_pop$mcEDct4$climate_WITCHco2
oldBAUdamSSP5 = PPstory5$v1x00$SIMUL$ed57$ssp5$noncoop_pop$mcEDct4$climate_WITCHco2

upBAUdamSSP3  = PPstory6$v1x00$SIMUL$ed57$ssp3$noncoop_pop$mcEDct4$climate_WITCHco2
upBAUdamSSP5  = PPstory6$v1x00$SIMUL$ed57$ssp5$noncoop_pop$mcEDct4$climate_WITCHco2
...
BKdf_PPstory6 = Build_Damage_DF_nty( YNETdam      = PPstory6$v1x00$SIMUL$ed57$ssp5$noncoop_pop$mcEDct4$climate_WITCHco2$damages_BURKEnSR$welfare_DICE$savings_fixed_converging$BAUdam$get_VARIABLE_nty("YNET",year_limit = 2200, unit = "Trill USD")
                                   ,YGROSSnodam = PPstory6$v1x00$OPTIM$ed57$ssp5$noncoop_pop$mcEDct4$climate_WITCHco2$damages_BURKEnSR$welfare_DICE$savings_fixed_converging$BAU$get_VARIABLE_nty(variable_name = "YGROSS", year_limit = 2200, unit = "Trill USD")
                                   ,Tlocal_dam  = PPstory6$v1x00$SIMUL$ed57$ssp5$noncoop_pop$mcEDct4$climate_WITCHco2$damages_BURKEnSR$welfare_DICE$savings_fixed_converging$BAUdam$get_TLOCALincr_nty %>% filter(year <= 2200)
                                   ,"SSP5 - Burke SR - post-Update"    )


BKdf_PPstory5 = Build_Damage_DF_nty( YNETdam      = PPstory5$v1x00$SIMUL$ed57$ssp5$noncoop_pop$mcEDct4$climate_WITCHco2$damages_BURKEnSR$welfare_DICE$savings_fixed_converging$BAUdam$get_VARIABLE_nty("YNET",year_limit = 2200, unit = "Trill USD")
                                     ,YGROSSnodam = PPstory5$v1x00$OPTIM$ed57$ssp5$noncoop_pop$mcEDct4$climate_WITCHco2$damages_BURKEnSR$welfare_DICE$savings_fixed_converging$BAU$get_VARIABLE_nty(variable_name = "YGROSS", year_limit = 2200, unit = "Trill USD")
                                     ,Tlocal_dam  = PPstory5$v1x00$SIMUL$ed57$ssp5$noncoop_pop$mcEDct4$climate_WITCHco2$damages_BURKEnSR$welfare_DICE$savings_fixed_converging$BAUdam$get_TLOCALincr_nty %>% filter(year <= 2200)
                                     ,"SSP5 - Burke SR - pre-Update"    )
...
plottigat = RICEx.plot.multimap(
  EXPdata   = list(
    ...
    "POSTUpdate Damages"    = BKdf_PPstory6  %>% select("n","t","year", "BKdamage") %>% filter(year == myyear) %>% rename( value= BKdamage)
    ,"PreUpdate Damages"  = BKdf_PPstory5 %>% select("n","t","year", "BKdamage") %>% filter(year == myyear) %>% rename( value= BKdamage)

...
#
#  > Improved charts for final paper here <
# 
#:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
...
plottigat <- RICEx.plot.lineplot(
  ...
  EXPdata   = list(
    ...
    ,"CBA non-coop" = PP5$ssp2$noncoop_pop$mcEDct4$climate_WITCHco2$damages_BURKEnSR$welfare_DICE$savings_fixed_converging$CBA$get_world_CIntensity_ty
    ,"BAU nodmg"   = PPstory5$v1x00$OPTIM$ed57$ssp2$noncoop_pop$mcEDct4$climate_WITCHco2$damages_BURKEnSR$welfare_DICE$savings_fixed_converging$BAU$get_world_CIntensity_ty
    ,"BAU dmg" = PPstory5$v1x00$SIMUL$ed57$ssp2$noncoop_pop$mcEDct4$climate_WITCHco2$damages_BURKEnSR$welfare_DICE$savings_fixed_converging$BAUdam$get_world_CIntensity_ty
...
## -----------------  SCC - BURKE SR + GAMMA 0 - Different SSPs  -------------------

#+++++++++++++++++++++++++++++++++++++++++
# We can do that for GAMMA = 0 because 
# SCC is identical among all cuntries
#+++++++++++++++++++++++++++++++++++++++++
...
ggplot() + 
  ...
  geom_line(data= PPstory5$v1x00$OPTIM$ed57$ssp1$coop_pop$mcEDct4$climate_WITCHco2$damages_BURKEnSR$welfare_disentangled$savings_fixed_converging$CBA__gamma0$get_SCC_nty
            ,aes(x=year, y=value, group=n, color = "SSP1")
            ,size = 1.2)  +
  ...
  geom_line(data= PPstory5$v1x00$OPTIM$ed57$ssp2$coop_pop$mcEDct4$climate_WITCHco2$damages_BURKEnSR$welfare_disentangled$savings_fixed_converging$CBA__gamma0$get_SCC_nty
            ,aes(x=year, y=value, group=n, color = "SSP2")
            ,size = 1.2)  + 
  
  geom_line(data= PPstory5$v1x00$OPTIM$ed57$ssp3$coop_pop$mcEDct4$climate_WITCHco2$damages_BURKEnSR$welfare_disentangled$savings_fixed_converging$CBA__gamma0$get_SCC_nty
            ,aes(x=year, y=value, group=n, color = "SSP3")
            ,size = 1.2)  + 
  
  geom_line(data= PPstory5$v1x00$OPTIM$ed57$ssp4$coop_pop$mcEDct4$climate_WITCHco2$damages_BURKEnSR$welfare_disentangled$savings_fixed_converging$CBA__gamma0$get_SCC_nty
            ,aes(x=year, y=value, group=n, color = "SSP4")
            ,size = 1.2)  + 
  
  geom_line(data= PPstory5$v1x00$OPTIM$ed57$ssp5$coop_pop$mcEDct4$climate_WITCHco2$damages_BURKEnSR$welfare_disentangled$savings_fixed_converging$CBA__gamma0$get_SCC_nty
            ,aes(x=year, y=value, group=n, color = "SSP5")
            ,size = 1.2)  + 
  
  # graphic details
  
  guides(colour=guide_legend(ncol=myncol)) +
  scale_color_manual(  values= i_want_pollos_colors(howmany_colors = 5) ) + 
  ...
  ggtitle(TeX("SCC under $\\gamma = 0$ - Different SSPs")) + 
  xlab("Year") +
  ylab("SCC [$/tCO2]")  
...
plottigat = RICEx.plot.multimap(
  EXPdata   = list(
    ...
    "CBA, $\\gamma = 0$"   = PP5$ssp2$coop_pop$mcEDct4$climate_WITCHco2$damages_BURKEnSR$welfare_disentangled$savings_fixed_converging$CBA__gamma0$get_MIU_nty %>% filter(year == myyear) %>%mutate(value = value * 100)
    ,"CBA, $\\gamma = 0.5$" = PP5$ssp2$coop_pop$mcEDct4$climate_WITCHco2$damages_BURKEnSR$welfare_disentangled$savings_fixed_converging$CBA__gamma0x5$get_MIU_nty %>% filter(year == myyear)  %>%mutate(value = value * 100)
    ,"CBA, $\\gamma = 1.45$"= PP5$ssp2$coop_pop$mcEDct4$climate_WITCHco2$damages_BURKEnSR$welfare_disentangled$savings_fixed_converging$CBA__gamma1x45$get_MIU_nty %>% filter(year == myyear)  %>%mutate(value = value * 100)
    ,"CBA, $\\gamma = 2$"   = PP5$ssp2$coop_pop$mcEDct4$climate_WITCHco2$damages_BURKEnSR$welfare_disentangled$savings_fixed_converging$CBA__gamma2$get_MIU_nty %>% filter(year == myyear)  %>%mutate(value = value * 100)
    ,"CBA NON-COOP"   = PP5$ssp2$noncoop_pop$mcEDct4$climate_WITCHco2$damages_BURKEnSR$welfare_DICE$savings_fixed_converging$CBA$get_MIU_nty %>% filter(year == myyear)  %>%mutate(value = value * 100)
...
plottigat <- RICEx.plot.boxplot(
  ...
  ,yLabel = "Local temperature increase [°C]"
  ,legend = "Scenarios"
  ,categories = 1
  ,colors_per_category = 5
  ,LaTeX_text = FALSE
